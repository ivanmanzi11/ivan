...
public class ElementalRun extends JPanel implements ActionListener, KeyListener {
    ...
    public ElementalRun(GameFrame parentFrame, Player.Sprite sprite, String backgroundPath) {
        ...
        bgMusic.playSound("src/main/java/Assets/background.wav", true);
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawString("Obstacles: " + totalObstaclesJumped, 20, 100);
        ...
        if (!gameOver && !isPaused) {
            ...
            for (int i = obstacles.size() - 1; i >= 0; i--) {
                ...
                if (CollisionDetector.checkCollision(player, obstacle)) {
                    ...
                    gameTimer.stop();
                    bgMusic.stop();
                    gameOverSound.playSound("src/main/java/Assets/gameover.wav", false);
                    parentFrame.gameEnded(score, currentLevel, totalObstaclesJumped);
                }
        ...
        if (!gameOver && !isPaused) {
            if (e.getKeyCode() == KeyEvent.VK_SPACE) {
                if (player.isOnGround() || player.getJumpCount() < Player.MAX_JUMPS) {
                    player.jump();
                    jumpSound.playSound("src/main/java/Assets/jump.wav", false);
                }
            }
        } else if (gameOver) {
